...
static inline int b64enc_internal(const unsigned char *data, unsigned int len, char *dest)
{
...
char *b64enc(char* dst, const unsigned char *data, unsigned int len)
{
    char *dest;

    /* malloc enough space to do something useful */
    if(!dst) dest = (char *) malloc(4 * len / 3 + 4);
	else dest = dst;
