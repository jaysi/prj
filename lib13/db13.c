...
bind_data:
    ...
    for(i = 0; i < ncol + 1; i++){//bind ifval here too!
        ...
        case DB_TY_BOOL:
        case DB_TY_INT:
            if(sqlite3_bind_int(LITE_ST(st), i==ncol?ACC_UP_IF_COL:col[i], (int32_t)(i==ncol?iflogic.ival:*val[i])) != SQLITE_OK){
                return e13_ierror(&db->e, E13_SYSE, "s", sqlite3_errmsg(LITE(db)));
            }
            break;
        ...
        case DB_TY_REAL:
            if(sqlite3_bind_double(LITE_ST(st), i==ncol?ACC_UP_IF_COL:col[i], (double)(i==ncol?iflogic.ival:*val[i])) != SQLITE_OK){
                return e13_ierror(&db->e, E13_SYSE, "s", sqlite3_errmsg(LITE(db)));
            }
            break;
