...
struct jdb_conf {
	/*
		the following elemets and equivalent/related elements in header
		are run-time constant and can be accessed without locking
		during the run-time. anyway any access to any element of the
		header needs refrence counting to avoid removing header while
		there is any refrence available. they are marked with RTC in
		the header/handle structure definition.
	*/
	jdb_hflag_t flags;
	jdb_bsize_t blocksize;
	uchar crc_type;
	uchar crypt_type;
...
struct jdb_handle {
	...
	struct jdb_conf conf;//lock by CONF

	struct jdb_hdr hdr;//lock by HDR
	struct jdb_map_list map_list;//lock by MAP
	struct jdb_table_list table_list;	/*only loaded tables will be
						put on this list,
						lock by TABLE*/
	//global indexing
	struct jdb_index0_blk_list index0_list;/*lock by INDEX0*/
	
	//undo
	struct jdb_changelog_list undo_list;/*lock by UNDO*/
	struct jdb_changelog_list redo_list;/*lock by UNDO*/
	
	//snapshots
	struct jdb_snapshot_list snapshot_list;/*lock by SNAP*/
};
