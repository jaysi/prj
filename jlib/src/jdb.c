...
static inline void _jdb_copy_conf_to_hdr(struct jdb_handle *h)
{
	...
	h->hdr.fav_load = h->conf.fav_load;
...
int jdb_close(struct jdb_handle *h)
{
	...
	don't forget to wait for handle (reader/writer)s.
