...
*/
	...
	pack(buf, "hclhcccchlhhhhhhhhhllllllt",
		hdr->magic,
		hdr->type,
		hdr->ver,
		hdr->flags,
...
int _jdb_unpack_hdr(struct jdb_hdr *hdr, uchar * buf)
{
	...
	unpack(buf, "hclhcccchlhhhhhhhhhllllllt",
		&hdr->magic,
		&hdr->type,
		&hdr->ver,
		&hdr->flags,
...
*/
	...
	for (j = 0; j < blk->max_ent; j += blk->base_len) {
	...
	_wdeb_data(L"buf: %s", buf+sizeof(struct jdb_cell_data_blk_hdr)+
			blk->bmapsize);
	...
	if(h->hdr.crc_type != JDB_CRC_NONE){
		...
		pack(buf + sizeof(struct jdb_cell_data_blk_hdr) -
			sizeof(uint32_t), "l", crc32);
	}
	return 0;
}
			...
			uchar * buf)
{
	...
	*/
	...
	_wdeb_data(L"read buf: %s", buf+sizeof(struct jdb_cell_data_blk_hdr) +
		blk->bmapsize);
