...
int _jdb_create_snapshot(struct jdb_handle *h)
{
	...
	blk->blockbuf = _jdb_get_blockbuf(h, JDB_BTYPE_SNAP, 0,
				JDB_CHANGE_BMAP_SIZE(h->hdr.snap_bent));
	if(!blk->blockbuf){
		free(blk);
		return -JE_MALOC;
	}

	blk->hdr = (struct jdb_snapshot_blk_hdr*)blk->blockbuf;
	blk->entry = (struct jdb_snapshot_blk_entry *)(blk->blockbuf+sizeof(struct jdb_snapshot_blk_hdr));
...
int _jdb_create_snapshot(struct jdb_handle *h, uchar* block, jdb_bid_t bid,
	...
	oldblock = _jdb_get_blockbuf(h, JDB_BTYPE_VOID, 0, 0);
