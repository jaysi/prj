...
int _jdb_create_col_typedef(struct jdb_handle *h, struct jdb_table *table)
{
	...
	_jdb_create_blk(h, table, blk, JDB_BTYPE_COL_TYPEDEF,
			sizeof(struct jdb_col_typedef_blk_hdr),
			sizeof(struct jdb_col_typedef_blk_entry),
			h->hdr.col_typedef_bent, &table->col_typedef_list,
			&ret);
...
int _jdb_load_col_typedef(struct jdb_handle *h, struct jdb_table *table)
{
	...
	for (i = 0; i < n; i++) {
		...
		blk->blockbuf = _jdb_get_blockbuf(h, JDB_BTYPE_COL_TYPEDEF, 0,
				JDB_CHANGE_BMAP_SIZE(h->hdr.col_typedef_bent));
		...
		blk->hdr = blk->blockbuf;
		blk->entry = (struct jdb_col_typedef_blk_entry*)(blk->blockbuf + 
				sizeof(struct jdb_col_typedef_blk_hdr));
		blk->change_bmap = blk->entry +
			sizeof(struct jdb_col_typedef_blk_entry)*
		...
		if (!table->col_typedef_list.first) {
			...
			table->col_typedef_list.last =
				table->col_typedef_list.last->next;
...
*/
void _jdb_free_col_typedef_list(struct jdb_handle* h, struct jdb_table *table)
{
	_jdb_free_blk_list(h, *table->col_typedef_list);
}
...
int _jdb_create_typedef(struct jdb_handle *h, struct jdb_table *table)
{
	...
	_jdb_create_blk(h, table, blk, JDB_BTYPE_TYPEDEF,
			sizeof(struct jdb_typedef_blk_hdr),
			sizeof(struct jdb_typedef_blk_entry),
			h->hdr.typedef_bent, &table->typedef_list,
			&ret);	
...
int _jdb_load_typedef(struct jdb_handle *h, struct jdb_table *table)
{
	...
	for (i = 0; i < n; i++) {
		...
		blk->blockbuf = _jdb_get_blockbuf(h, JDB_BTYPE_TYPEDEF, 0,
				JDB_CHANGE_BMAP_SIZE(h->hdr.typedef_bent));
		...
		blk->hdr = blk->blockbuf;
		blk->entry = blk->blockbuf + sizeof(struct jdb_typedef_blk_hdr);
		blk->change_bmap = blk->entry + sizeof(struct jdb_typedef_blk_entry) *
					h->hdr.typedef_bent);
...
void _jdb_rm_typedef_block(struct jdb_handle* h, struct jdb_table* table, jdb_bid_t bid){
	_jdb_rm_blk(h, &table->typedef_list, bid);
}

void _jdb_rm_col_typedef_block(struct jdb_handle* h, struct jdb_table *table, jdb_bid_t bid){
	_jdb_rm_blk(h, &table->col_typedef_list, bid);
}

void _jdb_free_typedef_list(struct jdb_handle* h, struct jdb_table *table)
{
	_jdb_free_blk_list(h, &table->typedef_list);
}
...
int jdb_typedef_flags(struct jdb_handle* h, wchar_t* table_name, jdb_data_t type_id, uchar* flags){
	...
	if((ret = _jdb_lock_table(h, table_name, JDB_LK_RD, &table))<0) return ret;
				...
				uint32_t col)
{
	...
	if(_jdb_lock_table(h, table_name, JDB_LK_RD, &table) < 0)
