...
int jdb_find_table(struct jdb_handle *h, wchar_t * name)
{
	...
	_jdb_lock_handle(h, JDB_LK_TABLE, JDB_LK_WR);
	
	if (h->magic != JDB_MAGIC){
		_jdb_unlock_handle(h, JDB_LK_TABLE, JDB_LK_WR);
		return -JE_NOINIT;
	}
	...
	if(!_jdb_lock_table(h, name, JDB_LK_RD, &table)){
		_jdb_unlock_table(h, name, JDB_LK_RD, &table);
		return 0;
	}
		 ...
		 uint16_t indexes)
{
	...
	_jdb_lock_handle(h, JDB_LK_NONE, JDB_LK_WR);
		
	if (h->magic != JDB_MAGIC){
		_jdb_unlock_handle(h, JDB_LK_NONE, JDB_LK_WR);
		return -JE_NOINIT;
	}
	...
	_jdb_init_table_lock(table);
	
	_lock_mx(&h->lock_mx);
	
	//check to see if table is not added... yet!
	for(t = h->table_list.first; t; t = t->next){
		if(!wcscmp(t->main.name, table->main.name)){
...
{
	_jdb_release_blockbuf(table->main.blockbuf);
}
...
int jdb_sync_table(struct jdb_handle *h, wchar_t * name)
{
	...
	if(table){
		ret =_jdb_sync_table_by_ptr(h, table);		
		return ret;
	}
	else{		
		return -JE_NOTFOUND;
...
int jdb_open_table(struct jdb_handle *h, wchar_t * name, uchar flags)
{
	...
	for(table = h->table_list.first; table; table = table->next){
		if(!wcscmp(table->main.name, name)){			
			return -JE_ISOPEN;
		}
	}
	...
	ret = _jdb_load_table(h, tid, flags);
