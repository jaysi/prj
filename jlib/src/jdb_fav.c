...
int _jdb_create_fav(struct jdb_handle *h, struct jdb_table *table)
{
	...
	_jdb_create_blk(	h, table, blk, JDB_BTYPE_TABLE_FAV,
				sizeof(struct jdb_fav_blk_hdr),
...
int _jdb_load_fav(struct jdb_handle *h, struct jdb_table *table)
{
	...
	for (i = 0; i < n; i++) {
		...
		blk->blockbuf = _jdb_get_blockbuf(h, JDB_BTYPE_FAV, 0,
					JDB_CHANGE_BMAP_SIZE(h->hdr.fav_bent));
		if(!blk->blockbuf){
			free(blk);
			return -JE_MALOC;
		}

		blk->hdr = (struct jdb_fav_blk_hdr*)blk->blockbuf;
		blk->entry = (struct jdb_fav_blk_entry *)(blk->blockbuf +
						sizeof(struct jdb_fav_blk_hdr));
...
void _jdb_rm_fav_block(struct jdb_handle* h, struct jdb_table *table, jdb_bid_t bid){
	_jdb_free_blk_list(h, &table->fav_list);
}
...
int _jdb_rm_fav(struct jdb_handle *h, struct jdb_table* table, jdb_bid_t bid)
{
	...
	if(!nful){
		...
		_jdb_track_change(blk->change_bmap,
			_jdb_calc_blk_entry_i(entry, blk->entry,
				sizeof(struct jdb_fav_blk_entry));
...
int _jdb_load_fav_blocks(struct jdb_handle* h, struct jdb_table* table){
	...
	if(!h->conf.fav_load) return 0;
	
	bid_list = (jdb_bid_t*)malloc(sizeof(jdb_bid_t)*h->hdr.fav_load);
	...
	for(i = 0; i < n; i++){
		if(!blk){
			blk = (struct jdb_cell_data_blk*)malloc(
					sizeof(struct jdb_cell_data_blk));
			if(!blk){
				ret = -JE_MALOC;
				goto end;
			}
			
			blk->blockbuf = _jdb_get_blockbuf(h, JDB_BTYPE_DATA,
					JDB_MEM_PLUS_CHANGE_BMAP);
			if(!blk->blockbuf){
				free(blk);
				return -JE_MALOC;
			}
			...
			blk->bitmap = 	blk->blockbuf +
					sizeof(struct jdb_cell_data_blk_hdr);
			blk->datapool = blk->blockbuf +
					sizeof(struct jdb_cell_data_blk_hdr) +
					blk->bmapsize;
			blk->change_bmap = blk->datapool +
					(h->hdr.blocksize - blk->bmapsize -
					sizeof(struct jdb_cell_data_blk_hdr));
		}

		blk->bid = bid_list[i];
		dtype = _jdb_get_block_dtype(h, blk->bid);
		if(dtype == JDB_TYPE_EMPTY) continue;
...
void _jdb_free_fav_list(struct jdb_handle* h, struct jdb_table *table)
{
	_jdb_free_blk_list(h, &table->fav_list);
}
